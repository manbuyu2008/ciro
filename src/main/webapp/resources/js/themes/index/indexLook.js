...
function tabClose() {
    ...
    //关闭当前
    $('.tabs-close').bind('click',function(){
        var subtitle = $(this).parent("li").children(".tabs-inner").children(".tabs-closable").text();
        $('#tabs').tabs('close',subtitle);
    })
}
//绑定右键菜单事件
...
function setSkin(value) {//两个参数，一个是cookie的名子，一个是值
    coco.utils.ajaxPostData("/admin/user/custom.vm?actionType=setSkin", {
            skin: value
        },
        function (data) {
            if (data) {
